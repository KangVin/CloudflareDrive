...
- 画像・テキストプレビュー
- **共有リンク**：有効期限付きでパスワード保護可能な公開共有リンクを作成、共有フォルダの閲覧
...
このシークレットは、パスワード保護された共有リンクのトークンを署名・検証するために使用されます。64文字のランダムな文字列を生成してください。設定しない場合はコード内のフォールバック値が使用されます（ローカル開発のみ）— **本番環境では必ず設定してください**。
...
| `POST`   | `/api/v1/shares`                    | 共有リンクの作成（`password` 対応）         |
| `DELETE` | `/api/v1/shares/:id`                | 共有リンクの取り消し                        |
| `POST`   | `/api/v1/s/:token/verify`           | 共有パスワード検証 → `verify_token` 返却    |
| `GET`    | `/api/v1/s/:token`                  | 公開アクセス（ファイル/フォルダ）           |
| `GET`    | `/api/v1/s/:token/browse/:folderId` | 共有フォルダ内のサブフォルダを閲覧          |
| `GET`    | `/api/v1/s/:token/download`         | 公開ファイルのダウンロード（`?vt=` 対応）   |
| `GET`    | `/api/v1/s/:token/download/:fileId` | 共有フォルダ内のファイルをDL（`?vt=` 対応） |

> パスワード保護された共有リンクには `X-Verify-Token` ヘッダー（ダウンロードは `?vt=` クエリパラメータも可）が必要です。`POST /verify` で取得してください。
